...
Startup timeline. From map construction, the events every build fires: `style.load`, the first `data` event carrying a tile, `load`, and the first `idle` after load; bundle import is timed separately from script tag insertion to evaluation. The builds disagree about whether a fade counts: mapbox-gl 1.13 holds `load` until its tile fade and `idle` until its label fade have finished, and the MapLibre builds fire `idle` within a few milliseconds of `load`. So each pass also measures startup a second time with `fadeDuration: 0`, and that `load` is the section's headline mark, the one that means the same amount of work in every build. Every startup begins from a blank page after a two-second settle so the previous context's teardown is not in the numbers. Then a pre-parsed FeatureCollection of 5,000 Berlin building footprints is added as a fill layer to the settled map and timed to the next idle.
